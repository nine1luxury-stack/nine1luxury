        ...
        >
            ...
            <div className="relative aspect-[3.5/5] overflow-hidden bg-surface-dark">
                ...
                <div className="absolute top-3 left-3 z-20 flex flex-col gap-2">
                    {isNew && (
                        <div
                            className="px-3.5 py-1.5 bg-ivory text-rich-black text-[7px] font-black rounded-full shadow-[0_8px_20px_rgba(0,0,0,0.3)] uppercase tracking-[0.2em] backdrop-blur-md flex items-center gap-1.5 animate-pulse"
                        >
                ...
                <Link href={`/products/${id}`} className="block w-full h-full relative overflow-hidden">
                    <Image
                        src={displayImage.includes('res.cloudinary.com') ? displayImage.replace('/upload/', '/upload/c_scale,w_400,q_auto/') : displayImage}
                        alt={name}
                        fill
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-3 group-hover:translate-y-0">
                        <div
                            className="bg-ivory/95 backdrop-blur-sm text-rich-black px-6 py-3 rounded-full font-bold text-[9px] uppercase tracking-[0.2em] shadow-2xl flex items-center gap-2 hover:bg-gold-500 transition-colors duration-300 transform hover:scale-105"
                        >
            ...
            <div className="p-4 md:p-5 space-y-2.5 flex-1 flex flex-col justify-between bg-gradient-to-t from-surface-card to-surface-card/80 relative">
                ...
                <div className="space-y-2">
                    ...
                    <Link href={`/products/${id}`}>
                        <h3 className="text-base md:text-lg font-bold text-ivory/90 group-hover:text-gold-300 transition-colors duration-300 line-clamp-1 font-cairo tracking-wide leading-tight">
                            {name}
                        </h3>
                    </Link>
                </div>
